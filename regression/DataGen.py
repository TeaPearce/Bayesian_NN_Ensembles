...
class DataGenerator:
	...
	def CreateData(self, n_samples, seed_in=5, 
		...
		if self.type_in=="regression":
		...
		elif self.type_in=="mauna":
			"""
			manoa time series dataset
			"""
			...
			if True:
				# X_train = np.atleast_2d(np.linspace(-2,2,200)).T
				# # X_train = np.atleast_2d(np.linspace(-0.5,0.5,100)).T
				# X_train += 1e-4
				# # X_train += np.random.normal(0,0.001,size=X_train.shape)
				# # X_train = np.atleast_2d(np.linspace(-2,2,300)).T
				# y_train = np.sin(X_train*40.)#*0.1 #+ X_train*0.1
				# X_train *= 4

				X_train = np.atleast_2d(np.linspace(-1,1,50)).T
				# X_train = np.atleast_2d(np.linspace(-8,3,500)).T
				# X_train = np.atleast_2d(np.linspace(-5,5,300)).T
				# X_train = np.atleast_2d(np.linspace(-2,2,6)).T
				# y_train = np.sin(X_train*4.)*0.2 + X_train*0.3 # good synthetic
				y_train = np.sin(X_train*4.)*0.2 #+ X_train*0.3
				# y_train = np.sin(X_train*10.)*0.2 + np.sin(X_train*5.+0.02)*0.1 + X_train*0.3 # great too
				# y_train = np.sin(X_train*20.)*0.2 + np.sin(X_train*10.+0.02)*0.1 + X_train*0.3
				y_train += np.random.normal(0,0.01,size=y_train.shape)


				# y_train = np.sin(X_train*20.)
